...
type AssigneeType = "UNASSIGNED" | "USER" | "TEAM";
...
type OptionTeam = { id: string; name: string | null };
...
export default function TaskFormModal({ mode, trigger, task, onSaved }: Props) {
  ...
  const [assigneeTeamId, setAssigneeTeamId] = React.useState<string | null>(
    null,
  );
  const [dueDate, setDueDate] = React.useState<string>("");

  const [users, setUsers] = React.useState<OptionUser[]>([]);
  const [teams, setTeams] = React.useState<OptionTeam[]>([]);
  ...
  const resetForm = React.useCallback(() => {
    ...
    setAssigneeTeamId(type === "TEAM" ? task?.assigneeTeamId || null : null);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      } else if (assigneeType === "TEAM" && assigneeTeamId) {
        payload.assigneeType = "TEAM";
        payload.assigneeId = assigneeTeamId;
      } else {
        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              {assigneeType === "TEAM" && (
                <Select
                  value={assigneeTeamId ?? "__none"}
                  onValueChange={(v) =>
                    setAssigneeTeamId(v === "__none" ? null : v)
                  }
                  disabled={loadingOptions}
                >
                  <SelectTrigger>
